...
import { pipeline, ProgressCallback, ProgressInfo } from '@huggingface/transformers';
    ...
    async function loadModel() {
      if (!model && open) {
        try {
          ...
          const progressCallback: ProgressCallback = (progressInfo: ProgressInfo) => {
            if ('progress' in progressInfo) {
              console.log(`Model loading progress: ${Math.round(progressInfo.progress * 100)}%`);
            }
          };
